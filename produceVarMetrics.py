...
def outputMutationFrequency(mutationFrequency,metricsFile,variantAlleleOrMutationFrequency,totalBases,perBases=10000): 
    ...
    metricsFile.write('%s\n'%'\t'.join(headers))
    for region in list(mutationFrequency.keys()):
        for feature in list(mutationFrequency[region].keys()):
    
            types4freq = ['synonymous','nonsynonymous','non-coding','totalMismatches']
            freqs4metrics = []
            for type in types4freq:  # @ReservedAssignment
            ...
            outMetrics = [region,
                          ...
                          '\t'.join([str(mutationFrequency[region][feature][type]) for type in types4freq]),  # @ReservedAssignment
                          '\t'.join(freqs4metrics)]                        
...
def outputShannonEntropyMetrics(metricsFile,shannonEntropy):
    ...
    for region in list(shannonEntropy.keys()):
...
def stats():
    ...
    nucVarFile = open(args.inputNucleotideVariance,'r')
    ...
    changedNucsLine = ''
    for regionName in list(changedNucs.keys()):
