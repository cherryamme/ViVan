...
# Release notes
# v0.65 - cleanup and added skip comment when parsing features file
# v0.66 - fixed a bug in which the synonymous change in an antisense feature would mark the amino acid position to be as if the feature is on the sense strand
...
class BaseVar:
    def __init__(self,baseLine,headerArgs):
        ...
        for allele in list(self.bases.keys()):
            self.bases[allele]=int(baseDict[allele])
        
        for base in list(self.bases.keys()):
...
def parseVariantFile(varFile,tests):
    ...
    for base in varFile:
        ...
        if divmod(bases,1000)[1]==0:
            print('\rVIVAN: Parsed %s/%s bases'%(bases,fileLen), end=' ')
            sys.stdout.flush()
...
def translateSeq(cds):
    ...
    finalCDS = cds
    try:
        translated = Seq.translate(cds,cds=True)  # @UnusedVariable
#         finalCDS = cds
    except TranslationError as e:  # @UnusedVariable
        try:
            reverseCDS = Seq.reverse_complement(cds)
            translated = Seq.translate(reverseCDS,cds=True)  # @UnusedVariable
            finalCDS = reverseCDS
            senseOrAnti = 'anti'
        except TranslationError as e:  # @UnusedVariable
...
#     regionSeqs = getSequenceFromVarFile(varFileName)
    ...
    for region in regions:
        ...
        numOpos=0
        positions = sorted(list(varDict[region].keys()),key=sortByPosition)
        for position in positions:
            numOpos+=1
            baseVar = varDict[region][position]
            if VALIDATE:
                if baseVar.ref!=regionSeq[int(position)-1].upper():
                    raise Exception('Position : %s, the reference allele in the variant file (%s) is not the same as the one in the reference sequence (%s).'%(position,baseVar.ref,regionSeq[int(position)-1]))            
            def sortByRatio(base):
                return baseVar.ratios[base]
            if isPassFilter(baseVar, filters):
                sortedVars = sorted(list(baseVar.ratios.keys()),key=sortByRatio,reverse=True)
...
#                             translated = Seq.translate(cds)
            ...
            if divmod(numOpos,500)[1]==0:
                print('\rVIVAN: Finished %s/%s positions'%(numOpos,len(positions)), end=' ')
                sys.stdout.flush()
