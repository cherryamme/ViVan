...
def getFileNames(fileList):
    ...
    fileLines = open(fileList)
    ls = popen('ls').read()
    fileNames = []
    for file in fileLines:  # @ReservedAssignment
...
def getColumnNum(fileName,columnTitle):
    file = open(fileName,'r')  # @ReservedAssignment
...
def getAlleleFreqFromBaseLine(baseDict,sortedAlleles,significantAlleles,AAchanges,region,MIN_RATE):
    ...
    if AAchanges:
        alleleFreqLine = {}
        for feature in list(AAchanges[region].keys()):
...
def compGroups(groupNames,groupDict,posDict, INCLUDE_FEATURES):
    ...
    if len(groupNames)==len(list(groupDict.keys())):
        combName= 'allGroupsComparison'
    popen('mkdir %s'%combName)
    #NOTE 这里创建了allGroupsComparison文件夹
    chdir('./%s'%combName)
    ...
    for groupName in groupNames:
        ...
        for outFile in outFiles:
            if INCLUDE_FEATURES:
                outFile.write('region\tfeature\tposition\treference\tcoverage\t%s\n'%('\t'.join(list(groupDict[groupName]['samples'].keys()))))
            else:
                outFile.write('region\tposition\treference\tcoverage\t%s\n'%('\t'.join(list(groupDict[groupName]['samples'].keys()))))    
   
        numOgroupFiles = len(list(groupDict[groupName]['samples'].keys()))
        for region in list(posDict[groupName].keys()):
            for position in sorted(posDict[groupName][region].keys()):
                
                numOpositionFiles = len([fileName for fileName in list(posDict[groupName][region][position]['files'].keys()) if posDict[groupName][region][position]['files'][fileName]!='.'])
                    ...
                    if otherGroup!=groupName:
                        if region in posDict[otherGroup]:
                            if position in posDict[otherGroup][region]:
                                filter=False  # @ReservedAssignment
                                break
                else:
                    filter=True  # @ReservedAssignment
                ...
                if common or filter:
                    ...
                    if features and features!=['.']:
                        for feature in features:
                            ...
                            for fileName in list(groupDict[groupName]['samples'].keys()):
...
#                print positionLines
    ...
    #NOTE 这里把allGroupsComparison文件夹移动到comparisons下
...
def getGroupConsenus(posDict,groupName,reference,refHeaders):
    ...
    for region in list(posDict[groupName].keys()):
        ...
        for position in list(posDict[groupName][region].keys()):
            #Validation
...
def compare():
    ...
    VERBOSE = args.verbose  # @UnusedVariable
    ...
    for filter in list(filters.keys()):  # @ReservedAssignment
        if filter=='coverage':
    ...
    for groupName in list(groupDict.keys()):
        for sample in list(groupDict[groupName]['samples'].keys()):
            ...
            for baseLine in file:
                ...
                MIN_RATE = 0
                for filter2check in list(filters.keys()):
                ...
                if PASS:
                    ...
                    if position not in posDict[groupName][region]:
                        ...
                        posDict[groupName][region][position]['files']=dict([(groupSamp,'.') for groupSamp in list(groupDict[groupName]['samples'].keys())])
            ...
            print('%s had %s positions: %s passed %s filters and %s did not'%(fileName,numOpositions,numOpassPos,' and '.join(list(filters.keys())),numOfailPos))
