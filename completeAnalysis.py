...
SUBVERSION= '91'
# Release notes:
# v0.45 : The script currently does not handle paired end illumina quality input
# v0.46 : Cleaned the script
# v0.47 : added -f argument to fastq-mcf so it outputs a file even when no trimming is done
# v0.48 : when the run includes paired end data, will set the REMOVE_N_READS argument to be false
# v0.49 : fixed cleanup to remove sai files as well
# v0.491 : fixed a bug where refusing clipping causes script to fail over remove N reads command

# The scripts directory, please change this to match the ViVan scripts directory
SCRIPTS_DIR = '/home/jc/ViVan/VV-source-py3'

# The bwa path, should be inside the ViVan scripts directory
BWA_PATH = '%s/bwa-0.7.8/bwa'%SCRIPTS_DIR
...
def waitForJobs(jobNums):
    ...
    while jobNums!=[]:
        ...
        timeWait+=SLEEP_TIME
        print(('\rwaited %s seconds'%timeWait), end=' ')
        sys.stdout.flush()
...
def validateGroupDict(groupDict):
    ...
    for group in list(groupDict.keys()):
...
def testQual(fileName):
    #NOTE 检查该文件是illumina还是sanger测序的
    print('VIVAN:QualityCheck:INFO: Testing file format of %s'%fileName)
...
def fixQualFormats(groupDict,QSUB,ILLUMINA_FORMAT,VERBOSE,LIGHT,QUEUE):
    ...
    jobNames = []
    for group in list(groupDict.keys()):
        for sample in list(groupDict[group]['samples'].keys()):
            fileNames = groupDict[group]['samples'][sample]
            print('ViVAN:QualityCheck:INFO: %s: files to check: %s'%(sample,','.join(fileNames)))
            qual = testQual(fileNames[0])
            if qual==format2fix:
                groupDict[group]['samples'][sample]=[]
                for i,fileName in enumerate(fileNames):
                    print('ViVAN:QualityCheck:INFO: %s is in %s format will now convert it into %s'%(fileName,format2fix,outFormat))
                    file2fixPrefix = sample
                    if len(fileNames)>1:
                        file2fixPrefix = '%s_R%s'%(file2fixPrefix,i+1)
                    fixCommand = 'python %s/convertFileFormat.py -i %s -p %s -f fastq-%s -t fastq-%s'%(SCRIPTS_DIR,fileName,file2fixPrefix,format2fix,outFormat)
                    commmands = [fixCommand]
                    # if QSUB is true, submit a job with all the commands
                    if QSUB:
                        jobName = 'FixQual-%s'%file2fixPrefix
                    ...
                    fixFileName = '%s.fastq-%s'%(file2fixPrefix,outFormat)
    ...
    while confLine:
        ...
        # get the filters that should be used in the group comparison section
            ...
            while not(confLine.startswith('#filters_end')):    
                filter,threshold = confLine.strip().split(delim)  # @ReservedAssignment
...
def printGroups(groupDict):
    # NOTE打印summary信息。print out the data parsed from the config file
...
def clipAndTrim(groupDict,QSUB,VERBOSE,LIGHT,QUEUE):
    ...
    for group in list(groupDict.keys()):
        for sample in list(groupDict[group]['samples'].keys()):
            fileNames = groupDict[group]['samples'][sample]
            print('VIVAN:ClipAndTrim:INFO: Now analyzing sample %s: (%s)'%(sample,','.join(fileNames)))
            if len(fileNames)!=1:
                print('ViVAN: Reads will not be clipped in sample %s since it is a paired end sample'%sample)
                continue
            else:
                outFiles = ['%s.trimmed.fq'%sample]
                clipNtrim = '%s/ea-utils/fastq-mcf -f -C 200000 -l 16 -q 30 -o %s %s/adapters.fa %s'%(SCRIPTS_DIR,' '.join(outFiles),SCRIPTS_DIR,fileNames[0])
...
def cleanup(groupDict,WEBSERVER):
    ...
    popen('mv *.sai *.sam *.bam* *.pileup %s 2>/dev/null'%(alignmentDir))
...
def analyze():
    ...
    if REMOVE_N_READS and not CLIP_AND_TRIM:
        REMOVE_N_READS=False    
        for group in list(groupDict.keys()):
            for sample in list(groupDict[group]['samples'].keys()):
                # only for single end
                if len(groupDict[group]['samples'][sample])==1:
                    finalFileName = '%s.filter.fq'%sample
                    popen('mv %s %s'%(groupDict[group]['samples'][sample][0],finalFileName))
